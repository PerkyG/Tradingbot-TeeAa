// api/telegram-webhook-v3.js - Complete Trading Journal Bot met alle fine-tuned vragen
...
async function sendWelcomeMessage(chatId) {
  const welcomeText = 
    "🦁 **Welkom bij je Complete Trading Journal Bot!**\n\n" +
...
async function askRandomQuestion(chatId, timeContext) {
  ...
  if (timeContext === 'morning') {
    selectedCategories = ['motivatie', 'doelen', 'voorbereiding', 'psychologie'];
  } else if (timeContext === 'afternoon') {
    selectedCategories = ['marktanalyse', 'strategie', 'discipline'];
  } else if (timeContext === 'evening') {
    selectedCategories = ['performance', 'risico', 'reflectie', 'persoonlijkeOntwikkeling', 'inzichten'];
  } else {
...
async function testNotionAPI(chatId) {
  try {
    const response = await fetch(`${API_URL}/api/trading-journal-v2`, {
      ...
      body: JSON.stringify({
        question: 'Test van Complete Trading Bot v3',
        answer: 'Dit is een test van het complete fine-tuned systeem!',
        category: 'Test',
        time_of_day: 'Test',
    ...
    if (response.ok) {
      await sendMessage(chatId, "✅ Test succesvol! Complete bot v3 is verbonden met Notion.");
    } else {
